...
| `research_category` | Mengelompokkan makalah penelitian atau dataset. |
...
- Menerapkan default `"allow"` hanya jika tidak ada kebijakan eksplisit dalam rantai leluhur.
...
- Memperlakukan `"allow"` sebagai izin untuk meng-ingest konten pada node tersebut.
...
Field `url` sangat penting karena menjadi pengenal kanonik untuk konten. Walau ringkasan menyampaikan makna konten, URL mengikat makna itu pada lokasi spesifik dan terverifikasi di web. Sistem AI menggunakan URL untuk deduplikasi, retrieval, validasi, dan cross-referencing dengan schema.org, sitemap, dan crawler.
...
| `content_type` | Arti |
|---|---|
| `research_paper` | Makalah ilmiah atau akademik. |
